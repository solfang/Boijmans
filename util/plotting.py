...
def plot_correlation(df,fpath=None):
    """
    Creates and shows a correlation plot
    :param df: pandas dataframe
    """
    corr = df.corr()
    ax = sn.heatmap(corr, annot=True, cmap="YlGnBu")  # linewidths=.5
    fig = ax.get_figure()
    if fpath:
        fig.savefig(fpath) 
    plt.show()
...
def plot_timeseries_histogram(series: pd.Series, freq="M", aggregate="count", xlabel="", ylabel="", title="",
    ...
    """
    Plots aggregated data over time.
    ...
    :param output_folder: if not None, save the aggregated data and plot to this file
...
def catplot(df: pd.DataFrame, groupby, freq="M", aggregate="count", xlabel="", ylabel="", title="",
            relative_to_group=False, label_string="", id_variable="id", output_folder="results/figures"):
    """
...
def catplot_area(df: pd.Series, groupby, freq="M", aggregate="count", xlabel="", ylabel="", title="",
                 relative_to_group=False, label_string="", id_variable="id", positions={},
    ...
    :param label_string: manual label string for the xaxis, used by strftime
    :param id_variable: column with identifiers, used for relative_to_group stuff
    :positions: dict of (category, index) for manually sorting the categories in the plot
    :param output_folder: if not None, save the aggregated data and plot into this folder
    ...
    data = piv.values.T  # no idea why I have to transpose. Pyplot docu says the data should be of shape (observationsxgroups), which is is without the .T
    labels = list(
...
def stacked_barchart(data, relative_to_group=True, title="", xtick_rotation=0, patch_y_offset=-0.06,
    ...
    for i, col in enumerate(data_used.columns):
        ...
        ax.bar(
            data_used.index, data_used[col], bottom=bottom, label=col)
    ...
    for i, total in enumerate(totals):
        ax.text(totals.index[i], total_data[i] + y_offset, round(total), ha='center',
                weight='bold', size=20)
    ...
    for i, bar in enumerate(ax.patches):
        ...
        ax.text(
            ...
            bar.get_height() + bar.get_y() + patch_y_offset,
